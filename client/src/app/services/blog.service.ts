...
export class BlogService {
  ...
  options;
  domain = this.authService.domain || this.vendorauthService.domain;
  ...
  createAuthenticationHeaders() {
    ...
    this.vendorauthService.loadToken();
    // Headers configuration options
    this.options = new RequestOptions({
      headers: new Headers({
        'Content-Type': 'application/json', // Format set to JSON
        'authorization': this.authService.authToken ||  this.vendorauthService.vendorauthToken// Attach token
      })
    });
  }
