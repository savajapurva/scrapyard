...
//--------------------------------
            // Find the current vendor that is logged in
            Vendor.findOne({ _id: req.decoded.vendorId }, (err, vendor) => {
              // Check if error was found
              if (err) {
                res.json({ success: false, message: err }); // Return error
              } else {
                // Check if username was found in database
                if (!vendor) {
                  res.json({ success: false, message: 'Unable to authenticate vendor' }); // Return error message
                } else {
                  // Check if the user who requested single blog is the one who created it
                  if (vendor.vendorname !== blog.createdBy) {
                    res.json({ success: false, message: 'You are not authorized to edit this blog.' }); // Return authentication reror
                  } else {
                    res.json({ success: true, blog: blog }); // Return success
                  }
...
//------------------------------------------------
            // Check who vendor is that is requesting blog update
            Vendor.findOne({ _id: req.decoded.vendorId }, (err, vendor) => {
              // Check if error was found
              if (err) {
                res.json({ success: false, message: err }); // Return error message
...
//--------------------------------------------------
            ...
            // Get info on vendor who is attempting to delete post
            Vendor.findOne({ _id: req.decoded.vendorId }, (err, vendor) => {
              // Check if error was found
              if (err) {
                res.json({ success: false, message: err }); // Return error message
...
//------------------------------------------------------
            ...
            // Get data from vendor that is signed in
            Vendor.findOne({ _id: req.decoded.vendorId }, (err, vendor) => {
              // Check if error was found
              if (err) {
                res.json({ success: false, message: 'Something went wrong.' }); // Return error message
                ...
                if (!vendor) {
                  ...
                  if (vendor.vendorname === blog.createdBy) {
                    ...
                    // Check if the user who liked the post has already liked the blog post before
                    if (blog.likedBy.includes(vendor.vendorname)) {
                      res.json({ success: false, message: 'You already liked this post.' }); // Return error message
                    } else {
                      // Check if user who liked post has previously disliked a post
                      if (blog.dislikedBy.includes(vendor.vendorname)) {
                        blog.dislikes--; // Reduce the total number of dislikes
                        const arrayIndex = blog.dislikedBy.indexOf(vendor.vendorname); // Get the index of the username in the array for removal
                        blog.dislikedBy.splice(arrayIndex, 1); // Remove user from array
                        blog.likes++; // Increment likes
                        blog.likedBy.push(vendor.vendorname); // Add username to the array of likedBy array
                        // Save blog post data
                        blog.save((err) => {
                          // Check if error was found
                          if (err) {
                        ...
                        blog.likes++; // Incriment likes
                        blog.likedBy.push(vendor.vendorname); // Add liker's username into array of likedBy
                        // Save blog post
                        blog.save((err) => {
                          if (err) {
                            res.json({ success: false, message: 'Something went wrong.' }); // Return error message
...
//-------------------------------------------------------------------
            ...
            // Get data of vendor who is logged in
            Vendor.findOne({ _id: req.decoded.vendorId }, (err, vendor) => {
              // Check if error was found
              if (err) {
                res.json({ success: false, message: 'Something went wrong.' }); // Return error message
                ...
                // Check if user was found in the database
                if (!vendor) {
                  res.json({ success: false, message: 'Could not authenticate vendor.' }); // Return error message
                } else {
                  // Check if user who disliekd post is the same person who originated the blog post
                  if (vendor.vendorname === blog.createdBy) {
                    res.json({ success: false, messagse: 'Cannot dislike your own post.' }); // Return error message
                    ...
                    if (blog.dislikedBy.includes(vendor.vendorname)) {
                      ...
                      if (blog.likedBy.includes(vendor.vendorname)) {
                        blog.likes--; // Decrease likes by one
                        const arrayIndex = blog.likedBy.indexOf(vendor.vendorname); // Check where username is inside of the array
                        blog.likedBy.splice(arrayIndex, 1); // Remove username from index
                        blog.dislikes++; // Increase dislikeds by one
                        blog.dislikedBy.push(vendor.vendorname); // Add username to list of dislikers
                        // Save blog data
                        blog.save((err) => {
                          // Check if error was found
                          if (err) {
                        ...
                        blog.dislikes++; // Increase likes by one
                        blog.dislikedBy.push(vendor.vendorname); // Add username to list of likers
                        // Save blog data
                        blog.save((err) => {
                          // Check if error was found
                          if (err) {
...
//---------------------------------------------------------------
              ...
              // Grab data of vendor that is logged in
              Vendor.findOne({ _id: req.decoded.vendorId }, (err, vendor) => {
                // Check if error was found
                if (err) {
                  res.json({ success: false, message: 'Something went wrong' }); // Return error message
                  ...
                  // Check if user was found in the database
                  if (!vendor) {
                    res.json({ success: false, message: 'Vendor not found.' }); // Return error message
                  } else {
                    // Add the new comment to the blog post's array
                    blog.comments.push({
                      comment: req.body.comment, // Comment field
                      commentator: vendor.vendorname // Person who commented
                    });
                    // Save blog post
                    blog.save((err) => {
                      // Check if error was found
